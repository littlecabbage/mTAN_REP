...
if __name__ == '__main__':
    ...
    if args.dataset == 'physionet':
        ...
        data_obj = get_swat_data(batch_size = args.batch_size)
    ...
    if args.enc == 'mtan_enc':
        ...
        print(f'''
        dim {dim}, 
        args.rec_hidden {args.rec_hidden}, 
    ...
    for itr in range(1, args.niters + 1):
        ...
        for train_batch, label in train_loader:
            ...
            # print(f"Training... current loss = {loss.shape} ")
            optimizer.zero_grad()
            loss.backward(loss.clone().detach())
            optimizer.step()
        writer.add_scalar('Loss per iteration', np.mean(loss_train), epoch)
        print(f"Iter {epoch}: Loss  = {np.mean(loss_train)}")
    ...
    # print(total_time)
        ...
        with torch.no_grad():
            ...
            # label = np.concatenate(label_test)
            print(f"loss test len: {len(loss_test), len(label_test)}")
            # roc_val = roc_auc_score(np.asarray(label_test, dtype = int), loss_test)
